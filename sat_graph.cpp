...
// Return cost of adding new vertex to path (number of new contradictions -- only computed at end) (1 = FALSE, 2 = TRUE)
...
/*** IMPLEMENTATIONS ***/
sat_graph_map::sat_graph_map(int nvar, int maxval, int plen, string conf) : construction_graph_map(nvar, 2, plen) { sat_init(conf); this->relative = this->circular = false; }
