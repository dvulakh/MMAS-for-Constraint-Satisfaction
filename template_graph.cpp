...
// Return the cost of adding new vertex to path (to be used as heta in ACS).
...
cost template_graph::cost_of(path& walk, vertex& nxt)
{
	// Return the cost
	return 0;
}

// Return whether new vertex may be added to path (hard constraint in ACS).
// Argument #1 (walk) is the current (incomplete) path constructed by the ants.
// Argument #2 (nxt) is the vertex being considered for next element.
// 
// Arguments are passed by reference to save time; neither should be modified.
...
bool template_graph::hard_constraint(path& walk, vertex& nxt)
{
...
template_graph_map::template_graph_map(int nvar, int max_val, int plen, string cons) :
